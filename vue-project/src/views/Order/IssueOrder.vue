<template>
  <el-row :gutter="20" style="margin-bottom: 10px">
    ...
    <el-col :span="8">
      <el-input v-model="searchId" placeholder="订单号搜索" clearable></el-input>
    </el-col>
    <el-col :span="8">
  ...
  <el-table :data="list" style="width: 100%">
    <el-table-column fixed="left" align="center" prop="orderNumber" label="订单号" width="100" />
    ...
    <el-table-column align="center" prop="express" label="物流" width="70" />
  ...
  <el-dialog title="编辑订单" v-model="editShow" width="40%">
    <el-form :model="order" label-width="80px">
      ...
      <el-form-item label="尺码">
        <el-select v-model="order.size" clearable placeholder="选择尺码大小">
  ...
  <el-dialog title="物流方式" v-model="expressShow" width="40%">
    <el-form :model="order" label-width="80px">
      <el-form-item label="物流">
        <el-select v-model="order.express" clearable placeholder="选择物流方式">
          <el-option v-for="item in expressType" :key="item.id" :label="item.type" :value="item.type" />
        </el-select>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" @click="confirmExpress">确定</el-button>
      </el-form-item>
    </el-form>
  </el-dialog>
</template>
...
export default {
  ...
  methods: {
    //获取订单信息
    async getIssueOrder() {
      const res = await RequestIssueOrder(this.pageSize, this.pageNo, this.searchId)
      if (res.data.code == 1) {
    ...
    async confirmExpress(){
      const res = await RequestChangeIssueOrderState(this.orderNumber,this.order.express)
      if (res.data.code == 1) {
        ElMessage({
          message: '发货成功!',
          type: 'success',
